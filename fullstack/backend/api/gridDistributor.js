...
const GridDistributorModel = require("../models/gridDistributorModel.js");
const {
  signup,
  login,
  protect,
  ...
  resetPassword,
  updatePassword,
} = require("./../services/authServiceDistributor.js");

const {isLoggedIn,loggedOut}= require("./../services/userServiceDistributor");
...
router.patch("/updateMe",bodyParser,protect,catchAsync(async (req,res,next)=>{
  //1. create error if user try to change password
   ...
   const updatedUser = await GridDistributorModel.findByIdAndUpdate(req.user.id, filteredBody,{
     new:true,
     runValidators:true
   })
...
router.get(
  ...
  catchAsync(async (req, res, next) => {     //need to add restrictTO middleware from authServices.js 
    const users = await GridOwnerModel.find();
    res.json(users);
  })
);
...
router.post(
  ...
  catchAsync(async (req, res, next) => {
    const user = new GridDistributorModel(req.body);     // need to add restrictTo middleware from authService.js
...
router.patch(
  ...
  catchAsync(async (req, res, next) => {         // need to add restrictTo middleware from authService.js
    const user1 = await GridDistributorModel.findByIdAndUpdate(req.params.id, req.body, {
      new: true,                                                               
      runValidators: true,
    });
...
router.delete(
  ...
  catchAsync(async (req, res, next) => {
    await GridDistributorModel.findByIdAndDelete(req.params.id);
    res.status(200).json({
      status: "success",
      data: null,
    });
...
router.get(
  "/:id",
  catchAsync(async (req, res, next) => {
    const users = await GridDistributorModel.findById(req.params.id);
