...
const GridOwner = require("../models/gridOwnerModel.js");
...
exports.signup = async (req, res, next) => {
  try {
    const newUser = await GridOwner.create({
...
exports.login = async (req, res, next) => {
  ...
  const user = await GridOwner.findOne({ email });
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  const freshUser = await GridOwner.findById(decoded.id);
...
exports.forgetPassword = async (req, res, next) => {
  ...
  const user = await GridOwner.findOne({ email: req.body.email });
...
exports.resetPassword = async (req, res, next) => {
  ...
  const user = await GridOwner.findOne({
...
exports.updatePassword = async (req, res, next) => {
  ...
  const user = await GridOwner.findById(req.user.id).select("+password");
